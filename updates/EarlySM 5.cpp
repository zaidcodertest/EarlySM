...
std::string subtract_from_string(std::string wordd, int ripper)
{
    std::string new_wordd;
    for (int take_off = 0; take_off < wordd.length() - ripper; take_off++)
    {
        new_wordd = new_wordd + wordd[take_off];
    }
    return new_wordd;
}
...
void output()
{
    if (openingscreen == "output(time)")
    {
        ...
        std::cout << "\n";
        std::cout << "Year: " << new_time.wYear << " Day: " << new_time.wDay << " Month: " << new_time.wMonth << " " << new_time.wHour << ":" << new_time.wMinute;
        std::cout << "\n\n";
        LAYOUT_EXCUSE = 1;
    }
